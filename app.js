...
const result = strings.filter(function (string) {
...
const convert = numbers.map(function (stringNumber) {
  return Number(stringNumber);
});
const square = convert.map(function (number) {
...
const str = alphabets.reduce(function (result, alphabet) {
...
const even = number.find(function (number) {
  return number % 2 === 0;
});
...
const index = ranks.findIndex(function (rank) {
  return rank === 7;
});
console.log(index);
...
const less = marks.some((mark) => mark < 45);
if (less) {
  console.log(true);
} else {
  console.log(false);
...
const negative = positive.every((number) => number > 0);
if (negative) {
  console.log(true);
} else {
  console.log(false);
